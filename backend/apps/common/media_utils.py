...
def ensure_placeholder() -> Path:
    """Create a small grey PNG placeholder if missing. Returns absolute path."""
...
def resolve_existing_relpath(relpath: str | None) -> str | None:
    """
    Return a relative media path that exists on disk.
    ...
    if alias and media_file_exists(alias):
        return alias
    # Case-insensitive match within the same directory (Windows uploads)
    ...
    if candidate.parent.is_dir():
        ...
        for sibling in candidate.parent.iterdir():
            if sibling.is_file() and sibling.name.lower() == target:
                return f"{cleaned.rsplit('/', 1)[0]}/{sibling.name}" if "/" in cleaned else sibling.name
    ensure_placeholder()
